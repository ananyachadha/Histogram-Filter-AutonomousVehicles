...
float sumAllGridItems(vector< vector <float> > grid) {
	float sum = 0;
...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	float sum = sumAllGridItems(grid);
	for(int i = 0; i < grid.size();i++){
		vector< float> row;
		for (int j = 0; j < grid[0].size(); j++){
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	for(int q = 0; q < height ; q++){
		for (int p = 0; p < width; p++){
			float gridValue = grid[q][p];
			for (int dx = -1; dx < (int)(window.size()-1); dx++){
				for (int dy = -1; dy < (int)(window[0].size()-1); dy++){
					float mult = window[dx+1][dy+1];
                    int new_q = (height + q + dy) % height;
                    int new_p = (width + p + dx) % width;
                    newGrid[new_q][new_p] += (mult * gridValue);
				}
			}
		}
	}
	return normalize(newGrid); ///////////IT FINALLY WORKS YES!
}
